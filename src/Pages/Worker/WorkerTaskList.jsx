...
const WorkerTaskList = () => {
  ...
  const { data: allTask, isLoading } = useQuery({
    queryKey: ["all-task", currentPage, itemsPerPage, sortBy, order],
    queryFn: async () => {
      const { data } = await axiosSecure(
        `/all-task?page=${currentPage}&size=${itemsPerPage}&sortBy=${sortBy}&order=${order}`
      );
      return data;
    },
  });
  ...
  return (
    <div className={`${containerClass} min-h-screen p-5`}>
      <div className="mb-5">
        <label
          htmlFor="sort"
          className={`text-sm font-medium ${textClass} mr-3`}
        >
          Sort by:
        </label>
        <select
